...
export const SalesOrderForm: React.FC<SalesOrderFormProps> = ({
  ...
  // Filter customers (type = CUSTOMER or BOTH)
  ...
  // When product changes, update unit price and tax rate
  const handleProductChange = (index: number, productId: string) => {
    const product = products.find(p => p.id === productId);
    if (product) {
      setValue(`items.${index}.unitPrice`, product.salesPrice || 0);
      // Optionally set a default tax rate (e.g., 18%)
      setValue(`items.${index}.taxRate`, 0.18);
    }
  };

  const handleFormSubmit = (data: OrderFormData) => {
    // Convert items to match SalesOrderInput
        ...
        <div className="border border-[#e5e7eb] rounded-md overflow-x-auto">
          <table className="min-w-full divide-y divide-[#e5e7eb]">
            ...
            <tfoot className="bg-[#f9fafb]">
              <tr>
                <td colSpan={3} className="px-3 py-2 text-right font-medium text-[#1a2332]">Subtotal</td>
                <td colSpan={3} className="px-3 py-2 text-right font-medium text-[#1a2332]">{formatCurrency(subtotal)}</td>
              </tr>
              <tr>
                <td colSpan={3} className="px-3 py-2 text-right font-medium text-[#1a2332]">Tax</td>
                <td colSpan={3} className="px-3 py-2 text-right font-medium text-[#1a2332]">{formatCurrency(taxAmount)}</td>
              </tr>
              <tr>
                <td colSpan={3} className="px-3 py-2 text-right font-bold text-[#1a2332]">Total</td>
                <td colSpan={3} className="px-3 py-2 text-right font-bold text-[#1a2332]">{formatCurrency(total)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
