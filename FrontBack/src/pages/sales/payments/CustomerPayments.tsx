...
export const CustomerPayments: React.FC = () => {
  ...
  // Updated handler with two-step posting
  const handleCreateSubmit = (data: CustomerPaymentInput, shouldPost: boolean) => {
    if (shouldPost) {
      createMutation.mutate(data, {
        onSuccess: (newPayment) => {
          setIsCreateModalOpen(false);
          postMutation.mutate(newPayment.id, {
            onSuccess: () => refetch(),
          });
        },
      });
  ...
  const handleUpdateSubmit = (data: CustomerPaymentInput, shouldPost: boolean) => {
    if (!selectedPayment) return;
    // Similar logic for update + post if needed
    // For simplicity, we only allow update without auto-post; can be extended.
